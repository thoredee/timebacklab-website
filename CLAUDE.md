...
**Seeding the table is a one-time manual paste** into the D1 dashboard Console (no `wrangler` on ARM64), same as `schema.sql`. Done 2026-07-02 (`SELECT COUNT(*)` returned 309). The seed file starts with `DROP TABLE IF EXISTS report_nodes`, so **re-running it wipes any edits made through the admin screen** — only re-run it to reset content to the `report-nodes.md` baseline. Keep the seed file **comment-free**: the D1 web Console rejects a paste whose first line is a `--` comment with "Requests without any query are not supported" (this bit us once). If `report-nodes.md` is later re-edited in bulk, regenerate the seed with the parser approach and note that re-pasting overwrites live edits.
...
- **API:** `GET /api/report?token=<submission_id>` (`src/api/report.js`, routed in `src/index.js`) — **public, no `X-Admin-Password`** (the token is the secret). Loads the submission from D1, parses `answers_json`, and assembles the report server-side, returning JSON. Also returns `report.tier` (the tier **headline / narrative / prescription** from a `TIER_CONTENT` map mirroring `js/quiz.js`'s `TIERS`, so the report opens with the same story as the purple results screen) and a per-section **`leakScore`** (0–100 severity = `((4-avg)/3)*100`, higher = more time lost) that drives the priority diagram.
- **Layout (redesigned 2026-07-02 — personalised report, not a Q&A transcript):** `report.html`/`js/report.js` render it client-side as: (1) a **purple top-card** with the tier headline + overall narrative/prescription, a **circular Timeback Score gauge** (mirrors the quiz gauge), tier badge and top-leak line; (2) a **"Your time leaks, in priority order"** diagram — the four areas ranked biggest-first as horizontal bars, each in its own brand colour, #1 tagged "Start here"; (3) one white block per section = section **summary first**, then the flagged responses under a **"What this means for you"** heading (report body text only — **the quiz question text and answer labels are deliberately NOT shown**), each with a volt-green left accent line; (4) a dark closing CTA. Each section carries a **per-category brand colour** (systems=cyan, delegation=pink, prioritisation=lilac, tech=volt) as a left accent strip + header dot, reused in the priority bars. Body background is brand paper `#FBF9F2` with a subtle volt/purple radial wash. Graceful states for missing token / not found / network error.

...
- **Seed:** [`schema/report-nodes-seed.sql`](../schema/report-nodes-seed.sql) — `report_nodes` table + all **309** INSERTs, generated from `report-nodes.md` by a parser script (192 summaries + 117 question nodes; Q10 Office correctly excluded, apostrophes escaped, `word_count` precomputed, `updated_at` NULL). File starts with `DROP TABLE IF EXISTS`, so **re-running it wipes admin edits** — it is a one-time initial load. Pasted into the D1 dashboard Console by hand (no `wrangler` on ARM64) and **confirmed live: `SELECT COUNT(*)` returned 309**. Note: the D1 web Console rejects a paste whose first line is a `--` comment ("Requests without any query are not supported"), so the seed file is kept comment-free; the apply/warning notes live here and in CLAUDE.md instead.
...
- **Shipped:** committed and pushed to `main` (commit `7059acc`); Cloudflare auto-deploys. Seed confirmed live (309 rows).
- **Open item (user):** log into the live admin screen when convenient and confirm it loads the 309 rows and that an edit saved through the modal persists on reopen.

### Editing report content later (durable workflow)
The user may occasionally ask to **review and update report content in the database**. Because there is no `wrangler` and no admin password available in a Claude session, Claude cannot read or write the live D1 directly. The workflow is:
1. Treat `docs/report-nodes.md` as the editable source/backup. Make the wording change there first (keep the brand voice: British English, no em dash, no Oxford comma, second person, strategic not tactical).
2. Produce **targeted `UPDATE` statements** for the affected rows only — e.g. `UPDATE report_nodes SET body = '...', word_count = N, updated_at = '<ISO>' WHERE id = '<composite-id>';` — for the user to paste into the D1 Console. **Never hand over a full re-seed for a small edit** (the seed drops the table and wipes every admin edit).
3. Alternatively, for one-off tweaks the user can just edit directly in the admin screen; if so, remember `report-nodes.md` will drift from the live DB and is no longer a byte-perfect backup.
Escape single quotes as `''` in the SQL. Keep the row `id`s exactly as the composite scheme (`sum-{section}-{size}-{role}-{tier}`, `{questionId}-{role}-{size}`).

## 2026-07-02 — Paid report access specced (Stripe + magic-link, TO BUILD)
Decided how buyers will pay for and unlock the detailed report, ahead of building it. **Spec/docs only, no code this pass.** Full brief: [`docs/payment-report-access-spec.md`](payment-report-access-spec.md).
- **Provider: Stripe** — owner is merchant of record, payouts to owner's bank, owner handles VAT (Stripe Tax later if needed). Chosen over a merchant-of-record (Paddle/Lemon Squeezy) for lowest fees/flexibility. **Amex is supported by default** — no separate Amex merchant account (a Stripe advantage over old processors).
- **Access model: magic link, no accounts.** On successful payment a signature-verified Stripe webhook marks the `submissions` row paid and mints an unguessable `report_token`; the buyer reaches their report via a private link, not a login. Data stays account-ready (keyed to email + `submission_id`) so accounts can be added later with no rework — deferred until repeat purchases / group reports / subscriptions justify them. "Customer account management" stays a placeholder in the admin menu.
- **Global cards + pricing:** a standard UK Stripe account accepts cards worldwide (US, India, etc.), money settles to the owner's GBP bank; international cards cost more (~3.25% + 20p + ~2% conversion). India caveat: one-off payments work but many Indian cards are disabled for international use by default (RBI), so higher decline rate — not fixable our side. **Pricing decision: one £ price + Stripe Adaptive Pricing** — owner sets/receives GBP, Stripe auto-displays the converted local currency (USD, INR, ...) at checkout. **Not** doing true per-country/purchasing-power pricing for v1 (needs self-maintained price tiers + geo logic, VPN-gameable) — noted as a later option only.
- **Report output format: still TBD** (on-site HTML vs downloadable PDF/doc). The gate doesn't depend on it — it only unlocks something keyed to the token.
- **Planned build:** additive `ALTER TABLE submissions` (paid, paid_at, stripe_session, amount_paid, currency, report_token) pasted into the D1 Console; new `POST /api/checkout`, `POST /api/stripe-webhook` (signature-verified, idempotent), `GET /api/report?token=` routes in the Worker; wire the results-screen "Order your report" button + a success/cancel page; add a Paid column/field to `admin.html`. Two Worker secrets (`STRIPE_SECRET_KEY`, `STRIPE_WEBHOOK_SECRET`), never in repo. Test-mode end-to-end before going live.
- **Owner to-do (when building starts):** decide the price, create + verify the Stripe account (guided), paste the two Stripe secrets into Cloudflare, paste the `ALTER TABLE` SQL into D1.
- This payment gate sits **in front of** the report generator; the generator itself is still the separate next phase below.

### Report builder — future action (needs thinking through)
The **report generator** is still unbuilt and is the next major phase to design. It will match a completed quiz submission to the right `report_nodes` rows (section summaries by section×size×role×tier from the score bands, plus question nodes for each flagged leak) and render the personalised detailed report. Open questions to decide before building: output format (on-site HTML page vs downloadable PDF/doc), where it is triggered (paid report request flow?), how tiers map to score bands per section, and how the generated report is stored/delivered. Spec groundwork is in [`docs/report-content-admin-spec.md`](report-content-admin-spec.md) (marked out of scope there).

## 2026-07-02 — Report builder v1 (concept test) built
First working version of the **report generator** — deliberately a concept test (pull the data together, render it, unique link) before any payment gate or bells and whistles. The "Order your report" button on the quiz results (the yellow `.report-section`) now links to a personalised report page.
- **New public API:** [`src/api/report.js`](../src/api/report.js) — `GET /api/report?token=<submission_id>` (routed in [`src/index.js`](../src/index.js)). **Public, no admin password** — the token is the unguessable `submission_id` (UUID) and acts as the magic link. It loads the submission from D1, parses `answers_json`, computes a **per-section tier** from each section's average answer (same `((avg-1)/3)*100` banding the quiz uses: ≤24 trapped · ≤49 overloaded · ≤74 stretched · else driver), picks the matching `sum-{section}-{size}-{role}-{tier}` summary node, flags questions answered **Heavy(1) or Moderate(2)** as leaks and pulls their `q{N}-{role}-{size}` question nodes, then returns the assembled report as JSON. Role mapping quiz→nodes: operator→boss, leader→lead, office→office, field→field; question id `op1`→`q1` etc. (strip role prefix). Office Q10 has no node — handled by only rendering ids that were found.
- **New page:** [`report.html`](../report.html) + [`css/report.css`](../css/report.css) + [`js/report.js`](../js/report.js). Built on the legal-page template (volt hero + off-white body), nav/footer copied verbatim from the site (hamburger included), `noindex`. Hero shows a **"Your private report link"** box with the current URL and a Copy button (magic link the user can save to return any time). The off-white body is filled client-side by `js/report.js`: a purple summary top-card (company · size · role · Timeback Score · tier badge · top leak), one white block per section (title, tier badge, top-leak highlight in pink, section summary, then flagged findings with the user's answer label, or a "no leaks flagged, a strength" note), and a dark closing CTA. Graceful states for missing token / not found / network error.
- **Quiz wiring:** [`js/quiz.js`](../js/quiz.js) now reads the `submissionId` returned by `POST /api/submit` (previously ignored), stores it in state, and points the "Order your report" button at `report.html?token=<submissionId>` (the anchor got id `report-cta-link`; updated in place when the POST resolves, since results render before the fetch completes).
- **Verified** locally via the static preview: page shell, hero, magic-link box + Copy button, and all graceful error states render with no console errors; the assembled report layout was confirmed by injecting representative report HTML (top-card, leak-highlighted section, findings, no-leak state, closing CTA all read well and on-brand). The live API/D1 path can only be confirmed once deployed (no `wrangler` on ARM64).
- **Shipped:** committed and pushed to `main`; Cloudflare auto-deploys.
- **Deliberately out of scope (bells and whistles for later):** payment gate (the Stripe + magic-link spec still sits in front of this), a dedicated hardened `report_token` (v1 reuses `submission_id`), PDF/downloadable output, email delivery, and tuning which questions count as "flagged". This pass is only to confirm a report generates, has a unique link, and reads well.

## 2026-07-02 — Report layout redesign (personalised, visual)
Reworked the report page from a Q&A transcript into a proper personalised report, on the user's feedback after seeing the live v1 read well.
- **Removed the questions + answer labels.** Section blocks no longer echo the quiz question text or the "Your answer: Heavy" pills. Each flagged item now shows **only its report body text**, grouped under a **"What this means for you"** sub-heading. Kept the volt-green vertical accent line on each (the user liked those).
- **Top card now opens with the results-screen story.** The purple top-card carries the same **tier headline** (e.g. "Overloaded. Not out of options.") and **overall narrative + prescription** the quiz-taker saw on the purple results screen, plus a **circular Timeback Score gauge** (mirrors the quiz gauge), tier badge and the "number one time leak" line. Required exposing this from the API — added a `TIER_CONTENT` map (headline/narrative/prescription per tier, copied from `js/quiz.js` `TIERS`) to [`src/api/report.js`](../src/api/report.js), returned as `report.tier`.
- **New priority-order diagram.** A white card, "Your time leaks, in priority order", ranks the four leak areas biggest-first as horizontal bars (each in its own brand colour), #1 tagged "Start here". Driven by a new per-section **`leakScore`** (0–100 severity = `((4-avg)/3)*100`, higher = more time lost) added to each section in the API. Ranking matches the row's `leak_category` (both key off lowest section average).
- **Brought to life on-brand while keeping content-first.** Off-white body switched to brand paper `#FBF9F2` with a very subtle volt/purple radial wash; each section gets a **per-category brand colour** (systems=cyan, delegation=pink, prioritisation=lilac, tech=volt) as a left accent strip + header dot, reused in the priority bars so the page reads as one system.
- **Files:** [`src/api/report.js`](../src/api/report.js) (tier content + per-section leakScore), [`js/report.js`](../js/report.js) (gauge builder, priority builder, restructured section render), [`css/report.css`](../css/report.css) (gauge, priority diagram, headline/narrative, section accents, paper background, responsive). No schema/D1 change — all derived from data already returned.
- **Verified** locally by rendering `js/report.js` against representative new-shape data (stubbed fetch) and screenshotting desktop + mobile with Playwright: top card + gauge + headline, priority bars, summary-then-"what this means for you" sections with green lines, per-section colour accents, and mobile stacking all render correctly and on-brand. Live API/D1 path confirmed only after deploy (no `wrangler` on ARM64).
- **Still deferred:** payment gate, hardened `report_token`, PDF/downloadable output, email delivery.

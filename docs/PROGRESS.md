...
**Open items**
- Confirm `ADMIN_PASSWORD` secret is set and admin.html can log in successfully end-to-end
- Confirm the D1 write is actually visible via `SELECT * FROM submissions` in the D1 Console (functional test already returned `{"ok":true,"submissionId":...}` from `/api/submit`, but hasn't been visually confirmed in D1 yet)
- Update CLAUDE.md's Deployment Pipeline / Folder Structure sections to reflect the Workers (not Pages) model, the new `.workers.dev` URL, and that `timebacklab.com` DNS is already connected
- No data export/reporting beyond the raw admin table yet (e.g. CSV export) — not requested this session
- No retention/deletion policy defined for stored submissions (relevant given Legal/Privacy page commitments)
